 ...
 */
public class ControllerFagAction extends ControllerAction implements EventHandler<ActionEvent> {

    // to avoid lots of casts, create an attribute that matches the instance type.
    private BasicView basicView;

    public ControllerFagAction(Model model, View view, Controller control) {
    ...
    private void setMenuHandlers() {
        ...
        // set event handler on the MenuOption item
        basicView.getMenuOption().setOnAction(e -> {
            view.setPause();
            view.removeMediaView();
            try {
                control.stopGame();
                basicView.setView("option",null);
            }
            catch(GameException err) {
                System.err.println(err.getMessage());
                System.exit(1);
